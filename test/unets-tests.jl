...
@test size(yhat[end]) == (256,256,3,1)   || error("Expected (256,256,3,1) but got $yhat[end]")
...
@test size(yhat[3]) == (64,64,128,1)   || error("Expected (64,64,128,1) but got $yhat[3]")
@test size(yhat[4]) == (32,32,256,1)   || error("Expected (32,32,256,1) but got $yhat[4]")
@test size(yhat[5]) == (16,16,512,1)   || error("Expected (16,16,512,1) but got $yhat[5]")
# @test size(yhat[6]) == (32,32,256,1)   || error("Expected (32,32,256,1) but got $yhat[6]")
# @test size(yhat[7]) == (64,64,128,1)   || error("Expected (64,64,128,1) but got $yhat[7]")
# @test size(yhat[8]) == (128,128,64,1)  || error("Expected (128,128,64,1) but got $yhat[8]")
# @test size(yhat[9]) == (256,256,32,1)  || error("Expected (256,256,32,1) but got $yhat[9]")
@test size(yhat[end]) == (256,256,3,1)  || error("Expected (256,256,3,1) but got $yhat[end]")
